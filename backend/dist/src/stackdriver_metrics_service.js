...
import { Interval } from './metrics_service.js';
const CLUSTER_NAME_URL = 'http://metadata.google.internal/computeMetadata/v1/instance/attributes/cluster-name';
const BASE = 'kubernetes.io';
const NODE_CPU_UTILIZATION_METRIC_TYPE = `${BASE}/node/cpu/allocatable_utilization`;
const CONTAINER_CPU_UTILIZATION_METRIC_TYPE = `${BASE}/container/cpu/limit_utilization`;
const CONTAINER_MEMORY_USED_METRIC_TYPE = `${BASE}/container/memory/used_bytes`;
export class StackdriverMetricsService {
    constructor(client, projectId, nodeNames) {
        this.client = client;
        this.projectId = projectId;
        this.nodeNames = nodeNames;
    }
    async getNodeCpuUtilization(interval) {
        const response = [];
        ...
        try {
            ...
            console.error('Unable to fetch Node CPU Utilization metrics:', err.details ? err.details : err);
        }
        return this.chronologicalSort(response);
    }
    async getPodCpuUtilization(interval) {
        const response = [];
        ...
        try {
            ...
            console.error('Unable to fetch Pod CPU Utilization metrics:', err.details ? err.details : err);
        }
        return this.chronologicalSort(response);
    }
    async getPodMemoryUsage(interval) {
        const response = [];
        ...
        try {
            ...
            console.error('Unable to fetch Pod Memory Utilization metrics:', err.details ? err.details : err);
        }
        return this.chronologicalSort(response);
    }
    // Sorts the list of TimeSeriesPoint objects ascending by time (in-place).
    chronologicalSort(points) {
        points.sort((a, b) => a.timestamp - b.timestamp);
        return points;
    }
    // Fetches the time series data aligned at 1m intervals
    async fetchTimeSeries(filter, interval) {
        try {
    ...
    getTimeSeriesInterval(interval) {
